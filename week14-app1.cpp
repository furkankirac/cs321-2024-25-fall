...
// ranges:    views, actions, algorithms
//   views:   lazy ranges (sequences), calculated in time of request
//   actions: same with views, but eager and in-place calculation. all calculation over range is done at the beginning of request
...
int main(int, char* [])
{
    ...
    { // Join two sequences by applying addition.
        ...
        const auto v2 = vector<int>{ 3, 2, 1 };
        auto rng = views::zip_with([](int a, int b) { return std::to_string(a + b); }, v1, v2);
        // ["7"s, "4"s, "8"s]
    }
